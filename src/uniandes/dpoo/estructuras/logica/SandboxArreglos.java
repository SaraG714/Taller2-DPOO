...
public class SandboxArreglos
{
    ...
    public void agregarEntero( int entero )
    {
    	...
    	int[] agregarIntArreglo = new int[arregloEnteros.length +1];
    	
    	for (int i = 0; i < arregloEnteros.length; i++) {
    		agregarIntArreglo[i] =  arregloEnteros[i];
    	}
    	
    	agregarIntArreglo[arregloEnteros.length] = entero;
    
    	arregloEnteros = agregarIntArreglo;
    ...
    {
        String[] agregarStrArreglo = new String[arregloCadenas.length + 1];
        
        for (int i = 0; i < arregloCadenas.length; i++) {
            agregarStrArreglo[i] = arregloCadenas[i];
        }
        
        agregarStrArreglo[agregarStrArreglo.length - 1] = cadena;
        
        arregloCadenas = agregarStrArreglo;
    ...
    public void eliminarEntero( int valor )
    {
    	...
    	int[] sinV = new int[noValor];
    	
    	int k = 0;
    	for (int i = 0; i < arregloEnteros.length; i++) {	
    		if(arregloEnteros[i] != valor) {
    			sinV[k] = arregloEnteros[i];
    			k ++;
    		}
    	}
    ...
    public void eliminarCadena( String cadena )
    {
    	...
    	String[] sinC = new String[noCadena];
    	
    	int k = 0;
    	for (int i = 0; i < arregloCadenas.length; i++) {	
    		if ((arregloCadenas[i] != null) && !(arregloCadenas[i].equals(cadena))) {
    			sinC[k] = arregloCadenas[i];
    			k ++;
    		}
    	}
    ...
    public void insertarEntero( int entero, int posicion )
    {
    	...
    	for (int i = 0; i < arregloNuevo.length; i++) {
    		if (i != posicion) {
    			arregloNuevo[i] = arregloEnteros[k];
    			k++;
    		} else if (i == posicion) {
    ...
    public void eliminarEnteroPorPosicion( int posicion )
    {
        if (posicion >= 0 && posicion < arregloEnteros.length) {
            ...
            int[] arregloSinV = new int[arregloEnteros.length - 1];
            

            for (int i = 0; i < posicion; i++) {
                arregloSinV[i] = arregloEnteros[i];
            }

            for (int i = posicion + 1; i < arregloEnteros.length; i++) {
                arregloSinV[i - 1] = arregloEnteros[i];
            }
            
            
            arregloEnteros = arregloSinV;
        }
    }
        ...
        for (int i = 0; i < arregloEnteros.length - 1; i++) {
            for (int j = i + 1; j < arregloEnteros.length; j++) {
                if (arregloEnteros[j] < arregloEnteros[i]) {
                    int reemplazo = arregloEnteros[i];
                    arregloEnteros[i] = arregloEnteros[j];
                    arregloEnteros[j] = reemplazo;
                }
            }
        }
    }
    ...
    public void organizarCadenas( )
    {
    	for (int i = 0; i < arregloCadenas.length-1; i++) { 		
    		for (int k = i+1; k < arregloCadenas.length; k++) {
    			if (arregloCadenas[k].compareTo(arregloCadenas[i]) < 0) {
    				String reemplazo = arregloCadenas[i];
    				arregloCadenas[i] = arregloCadenas[k];
    				arregloCadenas[k] = reemplazo;
    			}
    		}
    	}
    ...
    public int[] buscarEntero( int valor )
    {
    	...
    	int[] arregloPos = new int[numValor];
    ...
    public HashMap<Integer, Integer> calcularHistograma( )
    {
    	...
    	for (int i : arregloEnteros) {
    		if (!(histograma.containsKey(i))) {
    			...
    			histograma.replace(i, histograma.get(i)+1);
    		}
    	}
        return histograma;
    }
        ...
        for (int i = 0; i < otroArreglo.length - 1; i++) {
            for (int j = i + 1; j < otroArreglo.length; j++) {
                if (otroArreglo[j] < otroArreglo[i]) {
                    int reemplazo = otroArreglo[i];
                    otroArreglo[i] = otroArreglo[j];
                    otroArreglo[j] = reemplazo;
                }
            }
        }
